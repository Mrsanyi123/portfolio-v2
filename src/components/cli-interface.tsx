...
const COMMANDS = {
  ...
  about: () => `${DATA.name}\n${DATA.description}\n\n${DATA.summary}`,
  skills: () =>
    `Skills:\n${DATA.stackCategories.flatMap((category) => category.skills.map((skill) => skill.name)).join(", ")}`,
...
function formatCommandLine(line: string, currentTheme: string | undefined) {
  ...
  if (line.startsWith('$')) {
    return (
      <>
        <span className={currentTheme === 'dark' ? 'text-fuchsia-500' : 'text-fuchsia-600'}>dev@sanyi:~</span>
...
export function CliInterface({ onGuiCommand, onMinimize, onMaximize, onClose }: CliInterfaceProps) {
  ...
  useEffect(() => {
    ...
    setIsMounted(true);
    setOutput([
      isMobile ? 'Welcome to sanyi.dev CLI! 👋' : ASCII_ART + '\nWelcome to my portfolio CLI! 👋',
